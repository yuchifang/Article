...
export default function TopicBlock({
    ...
    const [articleInfoFilteredList, setArticleInfoFilteredList] = useState<
        ArticleProps[]
    >(ArticleInfoList)
    ...
    const [pageState, setPageState] = useState<DefaultPageStateType>(
        defaultPageState
    )

    const showPagination =
        articleInfoFilteredList.length / singlePageItemCount > 1
                                        ...
                                        {articleInfoFilteredList.map(
                                            (articleInfo, index) => (
                                                <ArticleInfo
                                                    ...
                                                    publicAt={
                                                        articleInfo.public_at
                                                    }
                                                    views={
                                                        articleInfo.total_hits
                                                    }
                                                />
                                            )
                                        )}
    ...
    const handleViewCountFilter = () => {
        setArticleInfoFilteredList((prevState) =>
            prevState.sort((articleA: ArticleProps, articleB: ArticleProps) =>
                Number(articleA.total_hits) > Number(articleB.total_hits)
                    ? -1
                    : 1
            )
        )
