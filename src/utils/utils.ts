...
export const _log = (variable: any) => {
    console.log('log', variable)
    return variable
}
export const packTagsString = (arr: any[]) =>
    arr.reduce(
        (totalTagsString, currentArticle) => [
            ...totalTagsString,
            ...currentArticle.tags,
        ],
        []
    )
export const countRepeatTag = (arr: any[]) =>
    arr.reduce((tagCount, currentTag) => {
        tagCount[currentTag] = tagCount[currentTag]
...
export function timestampToDate(timestamp: any) {
    ...
    if (timestamp.toString().length === 10) timestampNumber *= 1000
